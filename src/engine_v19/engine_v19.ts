...
function principalVariationSearch(
  ...
  for (const [r, c] of moveIterator) {
    ...
    if (moveIndex == 0) childResult = principalVariationSearch(game, depth - 1 + extension, ply + 1, -beta, -alpha, deadlineMs, usingNullWindow, restOfPrincipalVariation)[0]
    else {
      ...
      childResult = principalVariationSearch(game, searchDepth, ply + 1, -alpha - 1, -alpha, deadlineMs, true, restOfPrincipalVariation)[0]  // technically no longer the principal variation, but PV moves are probably still good in other positions
      ...
      if (alpha <= -childResult.eval && -childResult.eval <= beta && beta - alpha > 1) {
        failHigh++
        childResult = principalVariationSearch(game, depth - 1, ply + 1, -beta, -alpha, deadlineMs, false, restOfPrincipalVariation)[0]
      }
      else {
        confirmAlpha++
      }
