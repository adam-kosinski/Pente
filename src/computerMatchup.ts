...
} from "./engine_v14/model_v14";
import {
  makeMove as move15,
  type GameState as Game15,
} from "./engine_v15/model_v15";
import {
  makeMove as move16,
  type GameState as Game16,
} from "./engine_v16_diag_orthog/model_v16";
import {
  makeMove as move17,
  type GameState as Game17,
} from "./engine_v17/model_v17";
import {
  makeMove as move18,
  type GameState as Game18,
} from "./engine_v18/model_v18";
import {
  makeMove as move19,
  type GameState as Game19,
} from "./engine_v19/model_v19";
import {
  makeMove as move20,
...
const v = [
  ...
  { engine: engine20, create: create20, move: move20 }, // evaluate momentum
];
  ...
  while (!game1.isOver) {
    ...
    const engine =
      game1.currentPlayer === 0
        ? v[firstPlayer].engine
        : v[secondPlayer].engine;
...
export function runCompetition(
  ...
  for (let i = 0; i < nGames; i++) {
    ...
    const result = engineAFirst
      ? playGame(engineA, engineB, 15, msPerMove)
      : playGame(engineB, engineA, 15, msPerMove);
    ...
    // tally who won
    if (
      (engineAFirst && result.winner === 0) ||
      (!engineAFirst && result.winner === 1)
    ) {
      console.log("A won");
      winCounts["A"]++;
    } else {
...
export function generateFeatureCSV(
  nFeaturesPerGame: number,
  minMoveIndex: number = 0,
  maxMoveIndex: number = Infinity
) {
    ...
    for (let m = 0; m < moves.length; m++) {
      ...
      if (Math.abs(evaluation) !== Infinity) {
        ...
        game.currentPlayer === 0
          ? player0FeatureDicts.push(featureDict)
          : player1FeatureDicts.push(featureDict);
      }
    }
